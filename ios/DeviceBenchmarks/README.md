...
## No-inference hosted-process heartbeat

For an unexplained runner exit, independently opt in with `TEST_RUNNER_NOBONK_RUN_HOST_HEARTBEAT=1` and select `NoBonkQuickAccessTests/NoInferenceHostHeartbeatTests/testHostSurvivesFiveSecondsWithoutInference`. Use the same Release signing/build configuration as the failing detector test, with a fresh result path. This test creates no detector or pixel buffer, requests no camera, changes no app lifecycle state, and performs no network action. The normal production app host still initializes, including its ordinary UI/framework checks.

A detached worker emits a start record, five paired worker/MainActor progress records separated by one-second asynchronous sleeps, and an end record. Records include monotonic uptime, UTC time, whether execution is on Main, and Main's observed application state. Cancellation propagates to the worker. There is no busy spin or app runtime instrumentation. Progress lines use `NOBONK_HOST_HEARTBEAT_PROGRESS`; successful completion attaches `NoBonk-host-heartbeat-no-inference.json` and prints one `NOBONK_HOST_HEARTBEAT_COMPLETE` record.

Failure before completion points to a host/process/test-runner issue independent of model inference; passing only establishes five seconds of this configuration's survival. It cannot by itself identify an exit cause or absolve longer-running host behavior. Author validation is generic unsigned physical-iPhone Release `build-for-testing`; the parent owns execution on the phone.
